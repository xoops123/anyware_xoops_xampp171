...
{
	global $fck_allowed_extensions ,$fck_resource_type_extensions;
	...
	if ( !empty( $_FILES['upload'] ) && !empty( $_FILES['upload']['tmp_name'] ) && !empty( $_FILES['upload']['name'] ) && is_uploaded_file( $_FILES['upload']['tmp_name'] ) ) {
		$_FILES[FCK_UPLOAD_NAME] =  $_FILES['upload'];
	}
...
//Type check
	...
	if (empty($sType)){
		@unlink($_FILES[FCK_UPLOAD_NAME]['tmp_name']);
		SendResultsHTML( 1, '', '', 'Invalid type specified ' ) ;
	}
	if ( !array_key_exists( $sType, $GLOBALS['fck_resource_type_extensions'] ) ){
		@unlink($_FILES[FCK_UPLOAD_NAME]['tmp_name']);
		SendResultsHTML( 1, '', '', 'Invalid type specified ='.htmlspecialchars($sType,ENT_QUOTES) ) ;
	}
	if ($sType=='File'){
		$extension_array=$fck_resource_type_extensions['Image'];
		if ( in_array( $extension, $extension_array ) ){
			@unlink($_FILES[FCK_UPLOAD_NAME]['tmp_name']);
			SendResultsHTML( '202', '', 'Please Image  Type is to use image dialog', '('.htmlspecialchars(implode(',',array_values($extension_array)), ENT_QUOTES).') only' );
		}
	}else{
		$extension_array=$fck_resource_type_extensions[$sType];
		if ( !in_array( $extension, $extension_array ) ){
			@unlink($_FILES[FCK_UPLOAD_NAME]['tmp_name']);
			SendResultsHTML( '202', '', 'The '.$sType.' type is not allowed', '('.htmlspecialchars(implode(',',array_values($extension_array)), ENT_QUOTES).') only' );
		}
	}
	...
	if( ! in_array( $extension , array_keys( $fck_allowed_extensions ) ) ) {
		@unlink($_FILES[FCK_UPLOAD_NAME]['tmp_name']);
		SendResultsHTML( '1', '', 'Invalid file extension. allowed ('.htmlspecialchars(implode(',',array_keys($fck_allowed_extensions)), ENT_QUOTES).') only' , '' ) ;
	}
	...
	if( ! $trust_mode ) {
		...
		if( ! is_array( @$check_result ) || empty( $check_result['mime'] ) || stristr( $check_result['mime'] , $fck_allowed_extensions[ $extension ] ) === false ) {
			...
			//make thumbs
			if( defined( 'FCK_FUNCTION_AFTER_MKTHUMBS' ) && function_exists( FCK_FUNCTION_AFTER_MKTHUMBS ) ) {
				$thumbs_func_name = FCK_FUNCTION_AFTER_MKTHUMBS ;
				$thumbs_func_name( $new_filefullpath ) ;
			}
			// resize
...
function SendErrorXML( $number , $fileUrl = '' , $fileName = '' ,  $text = '' )
{
	...
	echo '<Connector><Error number="' . $number . '" text="' . htmlspecialchars( $text, ENT_QUOTES ) . '" /></Connector>' ;
...
{
	//add for CKeditor QuickUpload
	$sCommand=preg_replace( '?[^a-zA-Z]?' , '' , @$_GET['command'] ) ;
	$funcNum = intval(preg_replace( '?[^0-9]?' , '' , @$_GET['CKEditorFuncNum'] ))  ;
	if ( 'QuickUpload' == $sCommand )
	{
		if ($number && $number != 201) {
			echo "<script type='text/javascript'>window.parent.CKEDITOR.tools.callFunction($funcNum, '', 'Error ".$number.htmlspecialchars( $fileName , ENT_QUOTES ).htmlspecialchars( $text , ENT_QUOTES )." ');</script>";
		}
		else if ($number == 201){
